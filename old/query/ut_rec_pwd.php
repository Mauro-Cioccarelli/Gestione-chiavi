...
$hid=$_POST['h_id'];
/*/ ID UNICO
$sql=$mysqli->query("select ut_id from hs_utenti where ut_nome='{$_POST['h_id']}' limit 1");
if($sql->num_rows==0) $error="ID sconosciuto";
else { $res=$sql->fetch_array(); $hid=$res[0]; }
*/

if(!$error) {
$sql=$mysqli->query("select ut_id from keys_users where us_name='{$_POST['utente']}' and us_email='{$_POST['email']}' and hid='$hid'");
...
$email=$_POST['email'];
$md5_pwd=md5($new_pwd);
$mysqli->query("update keys_users set us_pwd_old=us_pwd, us_pwd='$md5_pwd', us_pw_cng='1' where us_id='$us_id' and hid='$hid'");
...
mail($email,"Nuova password KEYmanager - Non rispondere a questo messaggio",$testo,'From: "KEYmanager" <info@agenzianegri.it> \r\n');
}
